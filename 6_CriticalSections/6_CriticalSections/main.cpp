...
{
    std::cout << "Dimensionality of matrix A: " << kMatrixARows << " x " << kMatrixBRows << std::endl;

    GenerateMatrix(kMatrixARows, kMatrixBRows, matrix_a);

    std::cout << "Dimensionality of matrix B: " << kMatrixBRows << " x " << kMatrixBCols << std::endl;

    GenerateMatrix(kMatrixBRows, kMatrixBCols, matrix_b);

    std::thread* th[kMatrixARows][kMatrixBCols];
    for (int i = 0; i < kMatrixARows; i++) {
        for (int j = 0; j < kMatrixBCols; j++) {
            th[i][j] = new std::thread(MatrixMultiply, i, j, kMatrixBRows);
        }
    }
...
void ParallelThreads() {
    ...
    *counter2_ptr = 0;
    ...
    start_time3 = clock() - start_time3;
    printf("%f\n", (double)start_time3 / CLOCKS_PER_SEC);
